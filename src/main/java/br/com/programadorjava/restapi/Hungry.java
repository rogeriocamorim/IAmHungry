...
public class Hungry {
    ...
    public ResponseEntity<String> addProductToOrder(@RequestBody ClientOrder clientOrder) {
        ...
        if (clientOrder.getOrdemItems().isEmpty()) {
    ...
    @ResponseBody
    public ResponseEntity<List<ClientOrder>> getProductContainName(@RequestBody Customer customer) {
        if (customer == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
        }
        Optional<Customer> optCustomer = customerRepository.findByEmail(customer.getEmail());
        ...
        List<ClientOrder> listTemp = new ArrayList<>();

        optCustomer.get().getListaOrden()
        					.stream()
        					.map(
        							(ClientOrder clientOrder) ->
        							{
        								clientOrder.setOrdemItems(orderItemRepository.findByclientOrder_id(clientOrder.getId()));
    									return clientOrder;
        							}
        					).forEachOrdered((clientOrder) -> {
            listTemp.add(clientOrder);
        });
